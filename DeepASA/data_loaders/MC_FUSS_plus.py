...
class MCFUSSplusDataset(Dataset):
    """Spatialized multichannel FUSS-plus dataset"""
    def __init__(
        self,
        sp_wsj0_dir: str,
        dataset: str,
        ...
        """The Spatialized multichannel FUSS-plus dataset
        ...
        assert num_speakers in [2, 3, 4], num_speakers
        ...
        # self.wav_files = natsorted(self.wav_files)
    ...
    def __getitem__(self, index_seed: Union[int, Tuple[int, int]]):
        ...
        front = front.split('_')[-2]
        # mixed
        mix, sr = sf.read(self.wav_dir / f'mixed/filenum_{front}_mixed.wav')
        # clean
        clean = os.listdir(self.wav_dir / f'clean/filenum_{front}')
        wav_files = [file for file in clean if file.endswith(".wav")]
        wav_files = natsorted(wav_files)
        s1, sr = sf.read(self.wav_dir / f'clean/filenum_{front}' / wav_files[0])
        if len(wav_files) > 1: s2, sr = sf.read(self.wav_dir / f'clean/filenum_{front}' / wav_files[1])
        if len(wav_files) > 2: s3, sr = sf.read(self.wav_dir / f'clean/filenum_{front}' / wav_files[2])
        if len(wav_files) > 3: s4, sr = sf.read(self.wav_dir / f'clean/filenum_{front}' / wav_files[3])
        zeros_wav = np.zeros_like(s1.T)
        if len(wav_files) == 2: target = np.stack([s1.T, s2.T, zeros_wav, zeros_wav], axis=0)
        elif len(wav_files) == 3: target = np.stack([s1.T, s2.T, s3.T, zeros_wav], axis=0)
        elif len(wav_files) == 4: target = np.stack([s1.T, s2.T, s3.T, s4.T], axis=0)
        mix = mix.T
        ...
        return torch.as_tensor(mix, dtype=torch.float32), torch.as_tensor(target, dtype=torch.float32), torch.as_tensor(len(wav_files), dtype=torch.int64), paras

    def __len__(self):
        return len(self.files)


class MCFUSSplusDataModule(LightningDataModule):
    def __init__(
        self,
        sp_wsj0_dir: str,
        sample_rate: int = 16000,
        ...
        rank_zero_info("dataset: MCFUSSplusDataset")
