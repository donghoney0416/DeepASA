...
def _gen_extended_permutations(pred_num: int, spk_num: int, device: torch.device) -> Tensor:
    key = str(spk_num) + str(device)
    if key not in _ps_dict:
        # ps: all the permutations, shape [perm_num, spk_num]
        # ps: In i-th permutation, the predcition corresponds to the j-th target is ps[j,i]
        ps = torch.tensor(list(permutations(range(2 ** pred_num - 1), spk_num)), device=device)
        _ps_dict[key] = ps
    else:
        ps = _ps_dict[key]  # all the permutations, shape [perm_num, spk_num]
    return ps
...
def permutation_invariant_training2(
    mixed: Tensor,
    preds: Tensor,
    target: Tensor,
    metric_func: Callable,
    ...
    **kwargs: Any,
    ...
    if mode == "permutation-wise":
        ...
        ptarget = target.repeat_interleave(repeats=perm_num, dim=0)
        # shape of metric_of_ps [batch_size*perm_num] or [batch_size*perm_num, spk_num]
        metric_of_ps = metric_func(mixed, ppreds, ptarget)
        metric_of_ps = torch.mean(metric_of_ps.reshape(batch_size, len(perms), -1), dim=-1)
        # find the best metric and best permutation
        best_metric, best_indexes = eval_op(metric_of_ps, dim=1)  # type: ignore[call-overload]
        best_indexes = best_indexes.detach()
    ...
    # speaker-wise
    first_ele = metric_func(mixed, preds[:, 0, ...], target[:, 0, ...], **kwargs)  # needed for dtype and device
    metric_mtx = torch.empty((batch_size, spk_num, spk_num), dtype=first_ele.dtype, device=first_ele.device)
    metric_mtx[:, 0, 0] = first_ele
    for target_idx in range(spk_num):  # we have spk_num speeches in target in each sample
        for preds_idx in range(spk_num):  # we have spk_num speeches in preds in each sample
            if target_idx == 0 and preds_idx == 0:  # already calculated
                continue
            metric_mtx[:, target_idx, preds_idx] = metric_func(mixed, 
                preds[:, preds_idx, ...], target[:, target_idx, ...], **kwargs
            )
...
def extended_permutation_invariant_training(
    preds: Tensor,
    target: Tensor,
    metric_func: Callable,
    mode: Literal["speaker-wise", "permutation-wise"] = "speaker-wise",
    ...
    **kwargs: Any,
    ...
    # calculate the metric matrix
    batch_size, pred_num = preds.shape[0:2]
    batch_size, spk_num = target.shape[0:2]

    if mode == "permutation-wise":
        perms = _gen_extended_permutations(pred_num=pred_num, spk_num=spk_num, device=preds.device)  # [perm_num, spk_num]
        perm_num = perms.shape[0]
        if pred_num == 2:
            preds_1 = preds
            preds_2 = preds.sum(dim=1, keepdim=True)
            preds = torch.cat([preds_1, preds_2], dim=1)
        elif pred_num == 3:
            preds_1 = preds
            preds_2 = torch.cat([preds[:, [0, 1]].sum(dim=1, keepdim=True), preds[:, [0, 2]].sum(dim=1, keepdim=True), preds[:, [1, 2]].sum(dim=1, keepdim=True)], dim=1)
            preds_3 = preds.sum(dim=1, keepdim=True)
            preds = torch.cat([preds_1, preds_2, preds_3], dim=1)
        elif pred_num == 4:
            preds_1 = preds
            preds_2 = torch.cat([preds[:, [0, 1]].sum(dim=1, keepdim=True), preds[:, [0, 2]].sum(dim=1, keepdim=True), preds[:, [0, 3]].sum(dim=1, keepdim=True), preds[:, [1, 2]].sum(dim=1, keepdim=True), preds[:, [1, 3]].sum(dim=1, keepdim=True), preds[:, [2, 3]].sum(dim=1, keepdim=True)], dim=1)
            preds_3 = torch.cat([preds[:, [0, 1, 2]].sum(dim=1, keepdim=True), preds[:, [0, 1, 3]].sum(dim=1, keepdim=True), preds[:, [0, 2, 3]].sum(dim=1, keepdim=True), preds[:, [1, 2, 3]].sum(dim=1, keepdim=True)], dim=1)
            preds_4 = preds.sum(dim=1, keepdim=True)
            preds = torch.cat([preds_1, preds_2, preds_3, preds_4], dim=1)            

        # shape of ppreds and ptarget: [batch_size*perm_num, spk_num, ...]
        ppreds = torch.index_select(preds, dim=1, index=perms.reshape(-1)).reshape(
            batch_size * perm_num, *target.shape[1:]
        )
        ptarget = target.repeat_interleave(repeats=perm_num, dim=0)
        # shape of metric_of_ps [batch_size*perm_num] or [batch_size*perm_num, spk_num]
        metric_of_ps = metric_func(ppreds, ptarget)
