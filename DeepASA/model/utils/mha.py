...
class MHA(nn.Module):
    ...
    def __init__(
        ...
        if not self.cross_attn:
            ...
            self.Wq = linear_cls(embed_dim, embed_dim*num_heads, bias=qkv_proj_bias, **factory_kwargs)
            # self.Wkv = wqkv_cls(embed_dim, kv_dim, bias=qkv_proj_bias, **factory_kwargs)
            self.Wkv = nn.Sequential(
                nn.Conv1d(embed_dim, 2*kv_dim, kernel_size=3, padding=1, bias=qkv_proj_bias),
                nn.GLU(dim=1))
        if self.dwconv:
        ...
        self.out_proj = linear_cls(embed_dim*num_heads, embed_dim, bias=out_proj_bias, **factory_kwargs)
        ...
        if not self.cross_attn and self.num_heads_kv == self.num_heads:
            ...
            if self.cross_attn:
                if not self.return_residual:
                    ...
                    kv = self.Wkv(x_kv if x_kv is not None else x.transpose(1, 2)).transpose(1, 2)
                else:
                    if x_kv is not None:
